...
#! Name of the job:
#SBATCH -J OKVQA_VISUAL_DPR_TRAIN
...
options="--experiment_name \"OKVQA_WITCorpus(v2)(image_only)_VisualDPR_with_pretrained_ViT(WIT)_1ROI_with_text_based_vision\" \
    --config "configs/okvqa/wit/DPR_vision_with_ROI.jsonnet" \
    --accelerator auto --devices auto --strategy ddp \
    --mode train \
    --num_sanity_val_steps 0 \
    --reset --override \
    --modules freeze_doc_encoder_mapping_network \
    --opts train.trainer_paras.max_epochs=10000 \
             train.batch_size=30 \
             train.trainer_paras.val_check_interval=1000 \
             valid.batch_size=16 \
             train.trainer_paras.accumulate_grad_batches=2 \
             train.early_stopping_callback_paras.patience=3 \
             train.optimizer_config.optimizer_params.lr=0.00001 \
             train.optimizer_config.scheduler=none \
             model_config.num_ROIs=0 \
             train.load_model_path=\"/home/wl356/cvnlp_rds/wl356/projects/KBVQA/experiments/WIT_Pretraining_VisualDPR/train/saved_models/model_step_11000.ckpt\" \
             model_config.ModelClass=\"VisualDPRWithMultiModalDocsWithOnlyImages\" \
             model_config.multimodal_docs=1"
