...
options="--experiment_name \"OKVQA_VisualColBERT(10ROI_with_text_based_vision)_RAG(Flan-t5-large_with_text_based_vision)_epoch=10_scheduler=linear_lr=6e-5_retriever_lr=1e-5_bz=2_grad_accum=16_K=5_NLLLoss=1_DPOLoss=0.1(beta=0.1,detach,update_generator,tie_rank)\" \
    --config \"configs/rag/okvqa/RAG_colbert_with_vision_joint_training.jsonnet\" \
    --reset --override \
    --mode train \
    --opts train.trainer_paras.max_epochs=10 \
             train.batch_size=2 \
             ...
             train.trainer_paras.val_check_interval=200 \
             valid.batch_size=16 \
             ...
             train.optimizer_config.scheduler=linear \
             model_config.loss_ratio.dpo_loss=0.1 \
             model_config.loss_ratio.nll_loss=1 \
             model_config.loss_ratio.additional_loss=0 \
             model_config.dpo_beta=0.1 \
