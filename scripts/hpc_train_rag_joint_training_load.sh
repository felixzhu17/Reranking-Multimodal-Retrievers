...
options="--experiment_name \"OKVQA_VisualColBERT(10ROI_with_text_based_vision)_RAG(Flan-t5-large_with_text_based_vision)_epoch=30_lr=6e-5_retriever_lr=1e-5_constant_bz=2_grad_accum=16_K=5_RAGELoss(Approach5)=1_freeze(generator)_load(from_gqxl6hfa_1401)\" \
    --config \"configs/rag/okvqa/RAG_colbert_with_vision_joint_training.jsonnet\" \
    --reset --override \
    --mode train \
    --modules freeze_generator \
    --opts train.trainer_paras.max_epochs=30 \
             train.batch_size=2 \
             train.trainer_paras.accelerator=\"auto\" \
             ...
             train.trainer_paras.val_check_interval=400 \
             valid.batch_size=16 \
             train.early_stopping_callback_paras.patience=5 \
             train.optimizer_config.optimizer_params.lr=0.00006 \
             train.optimizer_config.retriever_lr=0.00001 \
             train.optimizer_config.scheduler=constant \
             train.load_model_path=\"/home/wl356/big_picture_rds/wl356/experiments/OKVQA_VisualColBERT(10ROI_with_text_based_vision)_RAG(Flan-t5-large_with_text_based_vision)_epoch=10_lr=6e-5_linear_bz=2_grad_accum=16_K=5/train/saved_models/model_step_1401.ckpt\" \
             model_config.RAVQA_loss_type=Approach5 \
             model_config.loss_ratio.nll_loss=0 \
