...
@register_executor
class RagExecutor(BaseExecutor, MetricsProcessor):
    def __init__(self,
        ...
        tokenizer=None,
        global_config=None,
        freeze_question_encoder_step=None,
        *args, **kwargs
        ):
        ...
        self.freeze_question_encoder_step = freeze_question_encoder_step
        logger.info(f"freeze_question_encoder_step: {self.freeze_question_encoder_step}")
        self.need_to_release_question_encoder = True if freeze_question_encoder_step is not None else False
        logger.info(f"self.need_to_release_question_encoder: {self.need_to_release_question_encoder}")
    ...
    def setup(self, stage):
        ...
        try:
            # copy all parameters from self.model.generator to self.model.generator_ref
            self.model.generator_ref.load_state_dict(self.model.generator.state_dict())
            # same for self.model.question_encoder
            self.model.question_encoder_ref.load_state_dict(self.model.question_encoder.state_dict())
            logger.info("Loaded the generator and question encoder to the ref models.")
        
        except Exception as e:
            logger.error(f"Error: {e}")
            logger.warning("No generator_ref or question_encoder_ref found. Skip loading.")
    ...
    def configure_optimizers(self):
        ...
        if 'freeze_question_encoder' in self.config.model_config.modules or self.need_to_release_question_encoder:
            # Freeze retriever
    ...
    def on_train_start(self):
        ...
        try:
            ...
            self.model.question_encoder_ref.eval()
            self.model.generator_ref.eval()
            logger.info("Set eval mode for reference question encoder and generator.")
        except:
            logger.info("No reference model found, skippping setting eval mode.")

    def training_step(self, sample_batched, batch_idx):
        # Release the question encoder if global step is larger than the threshold
        if self.need_to_release_question_encoder:
            if self.global_step > self.freeze_question_encoder_step:
                for n, p in self.model.named_parameters():
                    if 'generator' not in n:
                        logger.info(f"Release question encoder: {n}")
                        p.requires_grad = True

                self.need_to_release_question_encoder = False
    ...
    def on_validation_epoch_end(self):
        ...
        # try:
        #     # load state dict to reference models
        #     logger.info("Updating parameters to reference model...")
        #     # self.model.question_encoder_ref.load_state_dict(self.model.question_encoder.state_dict())
        #     self.model.generator_ref.load_state_dict(self.model.generator.state_dict())
        # except Exception as e:
        #     logger.error(f"Error: {e}")
