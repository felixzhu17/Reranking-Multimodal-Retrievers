...
class RagBlipExecutor(BaseExecutor, MetricsProcessor):
    ...
    def setup(self, stage):
        ...
        logger.error(
            f"Find {len(duplicated_rows)} duplicated rows in the passages dataset."
        )

        self.prepared_data.passages.id2doc = (
            df.drop_duplicates(subset=["passage_id"])
            .set_index("passage_id")
            .to_dict(orient="index")
        )
        for k, v in tqdm(
            self.prepared_data.passages.id2doc.items(), desc="formatting the passages"
        ):
        ...
        if not checkpoint_to_load or checkpoint_to_load == "":
            ...
            pretrained_dict = {
                k: v for k, v in state_dict_from_ckpt.items() if k in model_dict
            }
            logger.info(
            ...
            optimization_parameters = [
                {
                    "params": [
                        p
                        for n, p in self.model.named_parameters()
                        if "generator" in n and p.requires_grad
                    ],
                    "lr": optimizer_params.lr,
                    "initial_lr": optimizer_params.lr,
                },
                {
                    "params": [
                        p
                        for n, p in self.model.named_parameters()
                        if "generator" not in n and p.requires_grad
                    ],
                    "lr": retriever_lr,
    ...
    def training_step(self, sample_batched, batch_idx):
        ...
        for loss_name, loss_value in loss_dict.items():
            ...
            self.log(
                f"train/{loss_name}",
                loss_value.to(dtype=torch.float32),
                on_step=True,
                logger=True,
                sync_dist=True,
            )
        ...
        self.log(
            "train/loss",
            batch_loss.to(dtype=torch.float32),
            prog_bar=True,
            on_step=True,
            logger=True,
            sync_dist=True,
        ...
        for index, i in enumerate(labels):
            ...
            cleaned_i = [
                (
                    label
                    if label != -100
                    else self.decoder_tokenizer.tokenizer.pad_token_id
                )
                for label in i
            ]
            cleaned_i = torch.LongTensor(cleaned_i)
            decoded_label = self.decoder_tokenizer.tokenizer.decode(
            ...
            decoded_output = self.decoder_tokenizer.tokenizer.decode(
                output_sequence, skip_special_tokens=True
            )
            actual_output = self.decoder_tokenizer.tokenizer.decode(
    ...
    def evaluate_outputs(self, step_outputs, mode="test"):
        ...
        for step_output in step_outputs:
            ...
            batch_generation_outputs_for_docs += step_output[
    ...
    def logging_results(self, log_dict, prefix="test"):
        ...
        for metric, value in log_dict.metrics.items():
            if metric in ["precision", "recall", "gold_precision", "gold_recall"]:
                metrics_to_log[
                    f'{prefix}/{metric}_at_{log_dict.metrics["n_retrieved_docs"]}'
                ] = value
            else:
        ...
        if self.config.args.log_prediction_tables:
            ...
            local_table_path = os.path.join(
                self.config.test_dir, f"predictions_rank_{self.global_rank}.csv"
            )
