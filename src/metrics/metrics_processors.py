...
class MetricsProcessor:
    ...
    def compute_accuracy(self, module, data_dict, log_dict):
        ...
        for prediction in batch_predictions:
            ...
            if annotation is None:
                ...
                raise ValueError(
                    f"Annotation not found for question_id: {question_id}; the dataset might not be correct!"
                )
            ...
            if module.get("evaluate_alternative_answers", None):
                if prediction["answer"].lower() in annotation[
                    "answers"
                ] + annotation.get("alternative_answers", []):
        ...
        if module.get("evaluate_alternative_answers", None):
            log_dict.metrics["accuracy_with_alternative_answers"] = np.mean(
                np.array(alter_acc_array)
            )
        return log_dict
    ...
    def compute_exact_match(self, module, data_dict, log_dict):
        ...
        batch_generation_outputs_for_docs = data_dict[
        ...
        for answer_list, answer_proposals, answer_loss in zip(
            batch_answers, batch_generation_outputs_for_docs, batch_loss_with_doc_scores
        ):
            ...
            for i in range(n_beams):
                if i < len(ranked_answer_proposals):
                    if ranked_answer_proposals[i] in answer_list:
                        # find exact match
                        beam_count = 1
                exact_match_results["exact_match_at_{}".format(i + 1)].append(
                    beam_count
                )
    ...
    def compute_exact_match_with_numeric_values(self, module, data_dict, log_dict):
        """
        Compute exact match for Infoseek (numerical values can appear)
        """
        ##############################
        ##    Compute Exact Match   ##
        ##############################
        ...
        cleaner = TextCleaner()
        exact_match_results = {"accuracy": []}

        for answer_list, answer_proposal, numeric_ranges in zip(
            batch_answers, batch_predictions, batch_numeric_ranges
        ):
            correct = False
            # lower case
    ...
    def compute_retrieval_metrics(self, module, data_dict, log_dict) -> dict:
        ...
        for question_id, answer_list, docs in zip(
            batch_question_ids, batch_answers, retrieved_docs
        ):
        ...
        for answer_list, docs, answer_proposals, answer_loss in zip(
            batch_answers,
            retrieved_docs,
            batch_generation_outputs_for_docs,
    ...
    def compute_okvqa_scores(self, module, data_dict, log_dict) -> dict:
        ...
        try:
            ...
            vqa_helpers = EasyDict(
                {
                    "train": VQA(
                        module.vqa_data_path.annotation_files.train,
                        module.vqa_data_path.question_files.train,
                    ),
                    "test": VQA(
                        module.vqa_data_path.annotation_files.test,
            ...
            for quesType in vqaEval.accuracy["perQuestionType"]:
                metrics_to_log[f"accuracy_QuestionType_{quesType}"] = vqaEval.accuracy[
                    "perQuestionType"
                ][quesType]
            for ansType in vqaEval.accuracy["perAnswerType"]:
                metrics_to_log[f"accuracy_AnswerType_{ansType}"] = vqaEval.accuracy[
                    "perAnswerType"
                ][ansType]
            ...
            if self.trainer.state.stage in ["sanity_check"]:
                logger.info(
                    f"Failed to compute OKVQA scores: {e}"
    ...
    def compute_BLEU_scores(self, module, data_dict, log_dict) -> dict:
        ...
        for re in tqdm(batch_result):
            ...
            print(references[:3])
            for index, K in enumerate(Ks):
                output_scores = bleu.compute(
                    predictions=predictions[:K], references=references[:K]
                )
                ...
                result["translation_length"][index] += output_scores[
                    "translation_length"
                ]
