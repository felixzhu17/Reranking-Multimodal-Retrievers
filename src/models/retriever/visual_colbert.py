...
class VisualColBERTForPretraining(ColBERT):
    def __init__(
        self, name="bert-base-uncased", colbert_config=None, global_config=None
    ):
        ...
        vision_model_config = VisionModelConfigClass.from_pretrained(
            self.model_config.VisionModelVersion
        )
        self.vision_model = VisionModelClass.from_pretrained(
            self.model_config.VisionModelVersion, config=vision_model_config
        )
        ...
        if "freeze_colbert_doc_encoder" in self.model_config.modules:
            ...
            logger.warning(
                "freezing the ColBERT document encoder. If the query encoder is not separated, the query encoder is also frozen."
            )
...
class VisualColBERTForRetrievalMultipleMapping(ColBERT):
    def __init__(
        ...
        for proj_name, proj_config in self.model_config.vision_projections.items():
            ...
            if not checkpoint_to_load or checkpoint_to_load == "":
                ...
                pretrained_dict = {
                    k.replace("model.vision_projection.", ""): v
                    for k, v in state_dict_from_ckpt.items()
                    if k.startswith("model.vision_projection")
                }
                logger.info(
                    f"Load the following parameters from the given checkpoint: {pretrained_dict.keys()}"
                )
    ...
    def query(self, input_ids, attention_mask, image_features):
        ...
        for proj_layer in self.vision_projections:
            projected_embeddings = proj_layer(
                last_hidden_states
            )  # batch_size (x num_ROIs) x prefix_len*lm_embedding_size
            transformed_last_hidden_states.append(projected_embeddings)

        # batch_size x num_ROIs*num_proj x lm_embedding_size
        transformed_last_hidden_states = torch.cat(
            transformed_last_hidden_states, axis=1
        )
...
class ColBERTWithMultimodalDocs(ColBERT):
    ...
    def doc(self, input_ids, attention_mask, image_features=None, keep_dims=True):
        ...
        if self.multimodal_docs and image_features is not None:
            ...
            last_hidden_states = self.doc_vision_projection(
                last_hidden_states
            )  # bz (x num_ROIs) x 32*128
            ...
            image_mask = torch.ones(D.shape[0], D.shape[1] - mask.shape[1], 1).to(
                self.device
            )
...
class VisualColBERTForPretrainingWithTransformerMapping(ColBERT):
    def __init__(
        ...
        transformer_mapping_config_base = (
            self.model_config.transformer_mapping_config_base
        )
        ...
        self.vision_projection_input_linear = nn.Linear(
            self.vision_embedding_size, self.vision_embedding_size
        )
        self.vision_projection = BertEncoder(transformer_mapping_config)
        self.vision_projection_linear = nn.Linear(
            self.vision_embedding_size, self.lm_embedding_size
        )
    ...
    def query(self, input_ids, attention_mask, pixel_values):
        ...
        last_hidden_states = self.vision_projection_linear(
            last_hidden_states
        )  # bz x 50 x 128
...
class VisualColBERTForPretrainingWithTransformerMappingComposed(ColBERT):
    def __init__(
        ...
        self.vision_projection_input_linear2 = nn.Linear(
            self.vision_embedding_size, self.vision_embedding_size
        )
    ...
    def query(self, input_ids, attention_mask, pixel_values):
        ...
        first_token_hidden_states = self.vision_projection_input_linear(
        ...
        last_hidden_states = torch.cat(
            [first_token_hidden_states, last_hidden_states], dim=1
        )
        Q = last_hidden_states  # torch.cat([Q, last_hidden_states], dim=1) # concatenate hidden states
        # print("Q", Q.shape)
        # input("forward done")
...
class VisualColBERTForPretrainingWithShallowTransformerMappingComposedLoRA(
    ColBERT, ModuleUtilsMixin
):
    def __init__(
        self, name="bert-base-uncased", colbert_config=None, global_config=None
    ):
    ...
    def query(self, input_ids, attention_mask, pixel_values):
        ...
        # print("pixel_values", pixel_values.shape)
        bert_encoded_hidden_states = self.bert(
...
class VisualColBERTForPretrainingWithShallowTransformerMappingMAE(ColBERT):
    def __init__(
        ...
        self.vision_model.load_state_dict(
            torch.load("/data/maevit/mae_finetuned_vit_base.pth")["model"]
        )
...
class VisualColBERTForPretrainingWithShallowTransformerMappingComposedWithCrossAttn(
    ColBERT, ModuleUtilsMixin
):
    def __init__(
        self, name="bert-base-uncased", colbert_config=None, global_config=None
        ...
        # shallow transformer
        transformer_mapping_config.num_hidden_layers = self.model_config.get(
        ...
        self.vision_projection_input_linear2 = nn.Linear(
            self.vision_embedding_size, self.lm_hidden_embedding_size
        )
        ...
        self.vision_projection_linear = nn.Linear(
            self.lm_hidden_embedding_size, self.lm_embedding_size
        )
        ...
        if "enable_doc_encoder_lora" in self.model_config.modules:
            ...
            peft_config = get_peft_config(lora_config)
            self.model.bert = PeftModelForFeatureExtraction(
                self.model.bert, peft_config
            )
            self.model.bert.print_trainable_parameters()

            if "separate_question_encoder" in self.model_config.modules:
                logger.warning(
                    "separate_question_encoder is enabled. The query encoder is also initialized with LORA."
                )
                self.query_encoder = PeftModelForFeatureExtraction(
                    self.query_encoder, peft_config
                )
    ...
    def query(self, input_ids, attention_mask, pixel_values):
        ...
        Q = self.linear(bert_encoded_hidden_states)
        mask = (
            torch.tensor(self.query_mask(input_ids, skiplist=[]), device=self.device)
            .unsqueeze(2)
            .float()
        )
        Q = Q * mask
...
class VisualColBERTForRetrievalWithShallowTransformerMappingComposed(
    VisualColBERTForPretrainingWithShallowTransformerMappingComposedWithCrossAttn
):
    def __init__(
        self, name="bert-base-uncased", colbert_config=None, global_config=None
    ):
    ...
    def query(self, input_ids, attention_mask, pixel_values):
        ...
        # print("pixel_values", pixel_values.shape)
        bert_encoded_hidden_states = self.query_encoder(
            input_ids, attention_mask=attention_mask
        )[0]
        ...
        concat_hidden_states = torch.cat(
            [Q, first_token_hidden_states, last_hidden_states], dim=1
        )
