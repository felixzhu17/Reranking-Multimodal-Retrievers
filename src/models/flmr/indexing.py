...
def index_custom_collection(
    ...
    # Launch indexer
    with Run().context(
        RunConfig(nranks=nranks, root=index_root_path, experiment=index_experiment_name)
    ):

        # check if the index already exists
        index_name = f"{index_name}.nbits={nbits}"
        index_path = os.path.join(
            index_root_path, index_experiment_name, "indexes", index_name
        )

        if os.path.exists(index_path) and not overwrite:
            print(
                f"The index {index_path} exists. Skipping...\n Set `overwrite=True` to re-generate the index."
            )
            return index_path
