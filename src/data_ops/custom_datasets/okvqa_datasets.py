...
class OKVQADatasetForDPR(DPRBaseDataset, ModuleParser):
    ...
    def __getitem__(self, idx):
        def negative_sampling(
            question_id, pos_item_ids, negative_source=None, num_samples=1
        ):
...
class OKVQAWikipediaDatasetForDPR(DPRBaseDataset, ModuleParser):
    ...
    def __init__(self, config, dataset_dict):
        ...
        if len(self.passages.id2doc) == 0:
            ...
            ds = self.passages.dataset
            logger.info(f"Using {len(ds)} passage data...")
            self.passages.id2doc_train = (
                ds.to_pandas().set_index("id").to_dict(orient="index")
            )
            for k, v in tqdm(
                self.passages.id2doc_train.items(), desc="formatting the passages"
            ):
                self.passages.id2doc_train[k] = (
                    f"title: {v['title']} content: {v['text']}"
                )

            if (
                self.config.model_config.full_corpus_in_testing
                or self.config.model_config.full_corpus_in_training
            ):
                # Load full corpus only when needed
                ds = self.passages.full_dataset
                logger.info(f"Using {len(ds)} passage data...")
                self.passages.id2doc = (
                    ds.to_pandas().set_index("id").to_dict(orient="index")
                )
                for k, v in tqdm(
                    self.passages.id2doc.items(), desc="formatting the passages"
                ):
                    self.passages.id2doc[k] = (
                        f"title: {v['title']} content: {v['text']}"
                    )
            else:
                self.passages.id2doc = self.passages.id2doc_train
    ...
    def __getitem__(self, idx):
        def negative_sampling(question_id, answers, num_samples=1):
            ...
            """
            neg_items = []
            annotations = self.passages.annotations.get(
                str(question_id), {"passages": []}
            )["passages"]
            ...
            while len(neg_items) < num_samples:
                ...
                while True:
                    # if self.p is not None:
                    neg_item = random.sample(self.available_ids, 1)[
                        0
                    ]  # np.random.randint(low=0, high=self.n_passages-1, size=1)[0]
                    # else:
                    #     neg_item = np.random.choice(self.n_params.n_items, 1, p=self.p)[0]
                    # print(annotations, neg_item)
...
class OKVQAWITDatasetForDPR(DPRBaseDataset, ModuleParser):
    ...
    def __init__(self, config, dataset_dict):
        ...
        if len(self.passages.id2doc) == 0:
            ...
            self.passages.id2doc_train = (
                ds.to_pandas().set_index("passage_id").to_dict(orient="index")
            )
            ...
            for k, v in tqdm(
                ...
                self.passages.id2doc_train[k] = (
                    f"title: {v['page_title']} content: {v['passage_content']}"
                )
    ...
    def __getitem__(self, idx):
        def negative_sampling(
            question_id, answers, negative_source=None, num_samples=1
        ):
            """Generate negative samples for a query. ONLY used in training
            Args:
                user_item (int tensor): user id
            ...
            while len(neg_items) < num_samples:
                ...
                while True:
                    ...
                    if "use_self_negatives" in self.config.model_config.modules:
                        print(
                            "checking self negatives",
                            neg_item["source_name"],
                            negative_source,
                        )
                        if neg_item["source_name"] != negative_source:
                            VALID = False
