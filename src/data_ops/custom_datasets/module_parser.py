...
class ModuleParser():
    ...
    def PostProcessFLMRQuestionInputTokenization(self, data_to_process: EasyDict) -> EasyDict:
        """
        Post-processing for input tokenization
        """
        assert 'text_sequence' in data_to_process.keys()
    ...
    def PostProcessLLaVAQuestionInputTokenization(self, data_to_process: EasyDict) -> EasyDict:
        """
        Post-processing for input tokenization
        """
        assert 'text_sequence' in data_to_process.keys()
        ...
        for Q in Q_ids:
            ...
            max_len = len(Q)
            image_mask.append(
                [0] * img_token_index + [1] * img_token_span + [0] * (max_len-img_token_index-1)
            )
            instruction_mask.append(
                [1] * (sep_index + 1) + [0] * (max_len + img_token_span - sep_index)
            )
            question_mask.append(
                [0] * (sep_index + 1) + [1] * (img_token_index - sep_index - 1) + [0] * (max_len + img_token_span - img_token_index -1)
            )
    ...
    def PostProcessFLMRItemInputTokenization(self, data_to_process: EasyDict) -> EasyDict:
        """
        Post-processing for decoder input tokenization
        """
        assert 'text_sequence' in data_to_process.keys()
    ...
    def PostProcessQWenQuestionInputTokenization(self, data_to_process: EasyDict) -> EasyDict:
        """
        Post-processing for input tokenization
        """
        ...
        def preprocess(
            ...
            for i, source in enumerate(sources):
                ...
                for j, sentence in enumerate(source):
                    ...
                    if _img_token in second_part:
                        ...
                        instruction_and_question_part = second_part[second_part.index(_img_token)+2:]
                    else:
                        image_part = []
                    ...
                    if _separation[0] in instruction_and_question_part:
                        instruction_part = instruction_and_question_part[:instruction_and_question_part.index(_separation[0])]
                        question_part = instruction_and_question_part[instruction_and_question_part.index(_separation[0])+1:]
                    else:
                        instruction_part = []
                    ...
                    image_mask += [0] * len(first_part) + [1] * len(image_part) + [0] * len(instruction_part) + [0] * len(question_part) + [0] * len(last_part)
                    instruction_mask += [0] * len(first_part) + [0] * len(image_part) + [1] * len(instruction_part) + [0] * len(question_part) + [0] * len(last_part)
                    question_mask += [0] * len(first_part) + [0] * len(image_part) + [0] * len(instruction_part) + [1] * len(question_part) + [0] * len(last_part)
            ...
            instruction_masks = torch.tensor(instruction_masks, dtype=torch.int)[:, :batch_max]
            question_masks = torch.tensor(question_masks, dtype=torch.int)[:, :batch_max]
        ...
        for img, text_sequence in zip(img_path, text_sequences):
            ...
            sources.append([{
                "from": "user",
                "value": " ".join(input_sequence)
            }])
        
        encoding = preprocess(
            sources,
            self.tokenizer,
            self.config.model_config.max_source_length
        )
        input_ids, labels, attention_mask = encoding["input_ids"], encoding["labels"], encoding["attention_mask"]
        image_mask, instruction_mask, question_mask = encoding["image_mask"], encoding["instruction_mask"], encoding["question_mask"]
