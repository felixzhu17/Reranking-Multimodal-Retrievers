...
class LoadPreprocessedData(HFDatasetTransform):
    ...
    """
    def setup(self, data_path, passage_path, add_instruction=None, shuffle_splits=None, load_instruction=True, use_filtered_passages=False, num_data=None, num_passages=None, *args, **kwargs):
    ...
    def _call(self, *args, **kwargs):
        ...
        if self.num_data is not None:
            for split in self.num_data.keys():
                logger.info(f"Selecting {self.num_data[split]} examples from {split} split")
        ...
        if self.add_instruction is not None:
            for split in ['train', 'valid']:
                res[split] = res[split].map(add_instructions, num_proc=32, load_from_cache_file=False)
        ...
        if self.load_instruction:
            for split in ['train', 'valid']:
                ...
                if 'instruction' in res[split].column_names:
                    res[split] = res[split].map(combine_instruction_with_question, batched=True, num_proc=32, load_from_cache_file=False)
                else:
...
class LoadPreprocessedData_v2(HFDatasetTransform):
    ...
    """
    def setup(self, data_path, passage_path, image_root_folder=None, add_instruction=None, shuffle_splits=None, load_instruction=True,  num_data=None, num_passages=None, *args, **kwargs):
        self.data_path = data_path
        self.passage_path = passage_path
    ...
    def _call(self, *args, **kwargs):
        ...
        if "///" in self.data_path:
            ...
            self.data_path, data_sub_folder = self.data_path.split("///")[0], self.data_path.split("///")[1]
            data = load_dataset(self.data_path, data_sub_folder)
        else:
            data = load_dataset(self.data_path)

        if "///" in self.passage_path:
            self.passage_path, passage_sub_folder = self.passage_path.split("///")[0], self.passage_path.split("///")[1]
        ...
        if self.load_instruction:
            for split in all_splits:
                if "dummy" in res[split].column_names:
                    continue
                if 'instruction' in res[split].column_names:
                    res[split] = res[split].map(combine_instruction_with_question, batched=True, num_proc=32, keep_in_memory=False, load_from_cache_file=False)
                else:
                    logger.warning(f"Instruction not found in {split} split, skipping.")
        ...
        def concat_image_root_folder_with_img_path(example):
            example['img_path'] = os.path.join(self.image_root_folder, example['img_path'])
            return example

        for split in all_splits:
            if "dummy" in res[split].column_names:
                continue
            if 'img_path' in res[split].column_names and self.image_root_folder is not None:
                res[split] = res[split].map(concat_image_root_folder_with_img_path, num_proc=32, load_from_cache_file=False)
            # if 'img_path' not in res[split].column_names:
            #     logger.warning(f"img_path not found in {split} split, adding img_path from image_path")
            #     res[split] = res[split].add_column('img_path', res[split]['image_path'])
            ...
            for index, merge_from_data in enumerate(inputs):
                ...
                if passage_enabled:
                    if split == "valid_passages" and split not in merge_from_data:
                        ...
                        print(f"using test passages for {split} since {split} not found in {merge_from_data}")
                        merge_from_data[split] = deepcopy(test_ds)
                    if 'source_name' not in merge_from_data[split].column_names:
                        merge_from_data[split] = merge_from_data[split].add_column("source_name", [self.names[index]]*len(merge_from_data[split]))
...
class ConcatenateDatasets(HFDatasetTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for split in self.splits_to_process:
            ...
            all_split_data = []
            for input_data, enabled, negative_name in zip(inputs, self.concat_splits[split], self.negative_names):
                ...
                if split in input_data.keys() and enabled>0:
                    if "dummy" in input_data[split].column_names:
                        raise ValueError(f"dummy found in {input_data}, please disable this split in `concat_splits`.")
                    input_data_split = input_data[split]
                    if enabled > 1:
                        # duplicate input_data_split
                        input_data_split = concatenate_datasets([input_data_split for _ in range(enabled)])
                    if enabled < 1 and enabled > 0:
                        print(f"Randomly selecting {enabled} of {len(input_data_split)}")
                        # randomly select a subset
                        input_data_split = input_data_split.shuffle(seed=42).select(range(int(len(input_data_split)*enabled)))
                    
                    print(input_data_split)
                    if 'img_caption' in input_data_split.column_names:
                        input_data_split = input_data_split.remove_columns(['img_caption'])
                    if 'img_id' not in input_data_split.column_names:
                        input_data_split = input_data_split.add_column('img_id', [None]*len(input_data_split))
                    if negative_name is not None and negative_name != "":
                        input_data_split = input_data_split.add_column('use_negative_items', [negative_name]*len(input_data_split))
                        # print(input_data_split)
                        # input("here!")
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def add_text_based_vision(example):
            ...
            if module is not None:
                ...
                text_sequence = ' '.join([module.separation_tokens.start] + [caption] + [module.separation_tokens.end])
        ...
        for split in self.splits_to_process:
            if split in inputs.keys():
                ...
                ds_split = ds_split.map(add_text_based_vision, num_proc=32, load_from_cache_file=False)
