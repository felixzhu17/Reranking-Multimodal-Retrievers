...
class ExtractImageFeaturesWithVAE(BaseTransform):
    def setup(self, vae_model_config, *args, **kwargs):
        ...
        self.vae = VisionModelClass.from_pretrained(vae_model_config.VisionModelVersion).to(dtype=self.use_dtype)
        self.vae.eval()
        if torch.cuda.is_available():
            logger.info("using GPU for encoding...")
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def encode_images(batch, vision_model):
            ...
            # pixel_values = batch['pixel_values']
            pixel_values = torch.FloatTensor(pixel_values).to(device=vision_model.device, dtype=self.use_dtype)
            outputs = vision_model.encode(pixel_values).latent_dist.sample()
            image_features = outputs.reshape(outputs.shape[0], -1) # batch_size x latent_channels x 64 x 64 --> batch_size x -1
            batch['image_features'] = image_features.to(dtype=torch.float32)
            print(f"[Encoding Images] Memory Usage {psutil.Process(os.getpid()).memory_info().rss/1024/1024/1024:.4f} GB")
            return batch
