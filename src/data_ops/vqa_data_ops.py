...
class LoadVinVLFeatures(BaseTransform):
    ...
    def _call(self, *args, **kwargs):
        ...
        logger.info(
            "[Data Statistics] VinVL features {}".format(len(self.data.vinvl_features))
        )
...
class LoadOKVQAData(HFDatasetTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        vqa_helpers = EasyDict(
            {
                "train": VQA(
                    module_config.vqa_data_path.annotation_files.train,
                    module_config.vqa_data_path.question_files.train,
                ),
                "test": VQA(
                    module_config.vqa_data_path.annotation_files.test,
        ...
        for data_split, vqa_helper in vqa_helpers.items():
            ...
            for imgId in vqa_helper.imgToQA.keys():
                ...
                imgFilename = (
                    "COCO_" + dataSubType + "_" + str(imgId).zfill(12) + ".jpg"
                )
            ...
            for imgId, img_path, imgFilename in tqdm(img_list):
                ...
                image_ds = Dataset.from_pandas(image_df).rename_columns(
                    {"__index_level_0__": "id"}
                )
                ...
                for question_and_answer in related_question_and_answers:
                    ...
                    entry_data.answers = [
                        answer for answer in entry_data.answers if answer != ""
                    ]
                    ...
                    if module_config.add_VinVL_features:
                        # Read predictions from VinVL features
                        VinVL_prediction = self.data.vinvl_features.get(
                            entry_data.img_key_full, None
                        )
                        if not VinVL_prediction:
                            logger.error(
                                f"Image {entry_data.img_key_full} does not find associated VinVL features!"
                            )
                            raise KeyError(
                                f"Image {entry_data.img_key_full} does not find associated VinVL features!"
                            )
                    ...
                    if module_config.add_OCR_features:
                        text_annotations = self.data.ocr_features[
                            entry_data.img_key_full
                        ]["filtered_text_annotations"]
                        entry_data.img_ocr = text_annotations

                    self.data.okvqa_data[data_split].data_items[
                        entry_data.question_id
                    ] = entry_data
            ...
            split_df = pd.DataFrame.from_dict(
                self.data.okvqa_data[data_split].data_items, orient="index"
            )
            ...
            logger.info(
                "[Data statistics] split: {}  entries: {}".format(
                    data_split, len(self.data.okvqa_data[data_split].data_items)
                )
            )
...
class CropRegionOfInterestImages(HFDatasetTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        new_image_df = pd.DataFrame.from_dict(new_image_dict, orient="index").set_index(
            "id"
        )
        print(new_image_df)
...
class CropRandomRegionOfInterestImages(BaseTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for data_split in ["train", "test"]:
            for item in tqdm(
                self.data.okvqa_data[data_split].data_items,
                desc="generate random image crops",
            ):
                # print(item.question)
                selected_objects = []
                ...
                if self.module_config.even_split:
                    ...
                    for _ in range(self.module_config.max_objects):
                        ...
                        xmax = (
                            xmin + min_size + (w - xmin - min_size) * np.random.rand()
                        )
                        xmax = min(w, xmax)
                        ymin = min_size + np.random.rand() * (h - 2 * min_size)
                        ymax = (
                            ymin + min_size + (h - ymin - min_size) * np.random.rand()
                        )
                        ymax = min(h, ymax)
                        selected_objects.append(
                            {
...
class PrepareGoogleSearchPassages(HFDatasetTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        train_df = pd.DataFrame.from_dict(
            self.data.passages.id2doc_train, orient="index"
        )
...
class LoadGoogleSearchAnnotations(HFDatasetTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for data_split in ["train", "test"]:
            ...
            for annotation in tqdm(annotations):
                ...
                if self.data.passages.annotations.get(str(question_id), None) is None:
                    ...
                    logger.error(
                        f"Duplicated entries in annotation files! question id: {str(question_id)} collision!"
                    )
        ...
        for data_split in ["train", "test"]:
            ...
            for item in tqdm(
                self.data[data_split], desc="Adding knowledge annotation to samples"
            ):
                ...
                knowledge_item["pos_item_contents"] = [
                    self.data.passages.id2doc[str(passage_id)]
                    for passage_id in related_knowledge
                ]
                self.data.okvqa_data_with_dpr_output[data_split].data_items.append(
                    knowledge_item
                )
...
class ExtractImageFeaturesWithViT(BaseTransform):
    def setup(self, image_processor_config, vit_model_config, *args, **kwargs):
        ...
        vision_model_config = VisionModelConfigClass.from_pretrained(
            vit_model_config.VisionModelVersion
        )
        self.vision_model = VisionModelClass.from_pretrained(
            vit_model_config.VisionModelVersion, config=vision_model_config
        )
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def open_images(batch, image_processor=None):
            ...
            print(
                f"[Preparing Encoding] Memory Usage {psutil.Process(os.getpid()).memory_info().rss/1024/1024/1024:.4f} GB"
            )
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def load_cached_embeddings(batch):
            ...
            batch_cache_filenames = [
                os.path.join(self.cache_folder, f"{idx.split('/')[-1]}.safetensors")
            ...
            batch["image_features"] = image_features
            print(
                f"[Reading] Memory Usage {psutil.Process(os.getpid()).memory_info().rss/1024/1024/1024:.4f} GB"
            )
        ...
        def encode_images(batch, vision_model, image_processor):
            ...
            if all_prepared:
                ...
                print(
                    f"[Encoding] Memory Usage {psutil.Process(os.getpid()).memory_info().rss/1024/1024/1024:.4f} GB"
                )
                for cache_filename, image_feature in zip(
                    batch_cache_filenames, image_features
                ):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def encode_images(batch, rank, vision_model, image_processor):
            ...
            if all_prepared:
                ...
                for file_id, is_found, feature in zip(
                    batch_indices, found, image_features
                ):
                ...
                if len(actions) > 0:
                    ...
                    if res[0] != len(actions):
                        print("errors", res[1])
                    print(
                        f"[Rank {rank}] Successfully indexed {len(actions)} items into ES."
                    )
                else:
        ...
        remaining_image_dataset = image_dataset.filter(
            lambda x: x["found"], num_proc=16, load_from_cache_file=False
        )
        print("Already encoded number of images: ", len(remaining_image_dataset))
        image_dataset = image_dataset.filter(
            lambda x: not x["found"], num_proc=16, load_from_cache_file=False
        )
        ...
        if len(image_dataset) != 0:
            ...
            image_dataset_with_embeddings = concatenate_datasets(
...
@register_transform_functor
class ExtractImageFeaturesWithVAE(BaseTransform):
    def setup(self, vae_model_config, *args, **kwargs):
        """
        Example config:
        {
            input_column: "images",
        ...
        VisionModelClass = globals()[vae_model_config.VisionModelClass]
        self.vae = VisionModelClass.from_pretrained(
            vae_model_config.VisionModelVersion
        ).to(dtype=self.use_dtype)
        self.vae.eval()
        if torch.cuda.is_available():
            logger.info("using GPU for encoding...")
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        #     encoded = [
        #         self.image_transforms(img) for img in cropped_images
        #     ]

        #     pixel_values = torch.stack(encoded)
        #     batch['pixel_values'] = pixel_values
        #     return batch
        ...
        def encode_images(batch, vision_model):
            ...
            encoded = [self.image_transforms(img) for img in cropped_images]

            pixel_values = torch.stack(encoded)

            # pixel_values = batch['pixel_values']
            pixel_values = torch.FloatTensor(pixel_values).to(
                device=vision_model.device, dtype=self.use_dtype
            )
            outputs = vision_model.encode(pixel_values).latent_dist.sample()
            image_features = outputs.reshape(
                outputs.shape[0], -1
            )  # batch_size x latent_channels x 64 x 64 --> batch_size x -1
            batch["image_features"] = image_features.to(dtype=torch.float32)
            print(
                f"[Encoding Images] Memory Usage {psutil.Process(os.getpid()).memory_info().rss/1024/1024/1024:.4f} GB"
            )
            return batch
        ...
        #     batch_size=self.module_config.batch_size,
        #     num_proc=32,
        #     writer_batch_size=self.module_config.batch_size,
        # )
        image_dataset_with_embeddings = image_dataset.map(
            encode_images,
            batched=True,
...
@register_transform_functor
class ProcessImageWithViTProcessor(BaseTransform):
    def setup(self, image_processor_config, *args, **kwargs):
        self.module_config = EasyDict(kwargs)
        self.data = EasyDict()
        self.config = self.global_config
        self.image_processor_config = image_processor_config
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        input_column = self.module_config.input_column
        data_splits = self.module_config.data_splits

        for data_split in data_splits:
            for entry_data in self.data[input_column][data_split].data_items:
                img_path = entry_data.img_path
                print(img_path)
                img = cv2.imread(img_path)
                print(img)
                processed_image_input = self.image_processor(img, return_tensors="pt")
                pprint(processed_image_input)
                input()
