        ...
        for data_split, jsonl_path in module_config.vqa_data_path.items():
            ...
            with open(jsonl_path, "r") as f:
                for line in f:
                    ...
                    if (
                        self.data.vinvl_features.get(line_data["image_id"], None)
                        is None
                    ):
                        continue
                    if line_data.get("answer_eval", None) is not None:
                        ...
                        if isinstance(line_data["answer_eval"][0], dict):
                            ...
                            line_data["wikidata_value"] = line_data["answer_eval"][0][
                                "wikidata"
                            ]
                            line_data["wikidata_range"] = line_data["answer_eval"][0][
                                "range"
                            ]
                            line_data["answer_eval"] = []
            ...
            # read withkb data
            annotation_file_path = module_config.passage_annotation_file_path.get(
                data_split, None
            )
            ...
            ds_split = ds_split.rename_columns(
                {"data_id": "question_id", "answer": "answers"}
            )
            ...
            def add_features(entry_data):
                if module_config.add_VinVL_features:
                    ...
                    VinVL_prediction = self.data.vinvl_features.get(
                        entry_data["image_id"], None
                    )
                    if not VinVL_prediction:
                        logger.error(
                            f"Image {entry_data['image_id']} does not find associated VinVL features!"
                        )
            ...
            if num_examples_limit > 0 and len(ds_split) > num_examples_limit:
                ...
                logger.info(
                    f"Select {num_examples_limit} examples from {data_split} split"
                )
...
class CropRegionOfInterestImagesForInfoSeek(HFDatasetTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for data_split in ["train", "val"]:
                ...
                if len(selected_objects) == 0:
                    ...
                    if len(selected_objects) > self.module_config.max_objects:
                        selected_objects = selected_objects[
                            : min(self.module_config.max_objects, len(selected_objects))
                        ]
            ...
            cropped_ds_split = ds_split.map(
                crop_ROI, desc="crop_ROI", load_from_cache_file=False
            )
        ...
        self.out_data["images"] = concatenate_datasets(
            [self.data["images"], additional_image_dataset]
        )
            ...
            for data_split in ["train", "val"]:
                ...
                def search_wiki_passage_with_entity_text(example):
                    ...
                    if resp["hits"]["total"]["value"] > 0:
                            ...
                            if not found and example.wikidata_value is not None:
                                ...
                                for number in all_numbers:
                                    try:
                                        number = float(number)
                                        if (
                                            abs(number - example.wikidata_value) < 0.01
                                        ):  # >= float(example.wikidata_range[0]) and number <= float(example.wikidata_range[1]):
                                            print(
                                                "!! Found answer",
                                                number,
                                                "==",
                                                example.wikidata_value,
                                            )
                                            found = True
                                            gold_doc_ids.append(retrieved_doc["_id"])
                                            gold_doc_contents.append(passage_text)
                        ...
                        logger.error(
                            f"Cannot find a passage for {example.question_id}: {query}"
                        )

                    return example

                ds_split = ds_split.map(
                    search_wiki_passage_with_entity_text,
                    desc="search_wiki_passage_with_entity_text",
                ...
                logger.info(
                    f"Before removing examples without passages: {len(ds_split)}"
                )
                ds_split = ds_split.filter(
                    lambda x: len(x["pos_item_ids"]) != 0,
                    desc="remove_examples_without_pos_item_ids",
                    num_proc=128,
                )
                logger.info(
                    f"After removing examples without passages: {len(ds_split)}"
                )
    ...
    def _call(self, inputs):
        ...
        def _caption_with_blip(batch_examples):
            ...
            if self.save_to_disk_column is not None:
                ...
                paths_to_save = [p + ".txt" for p in paths_to_save]
            ...
            if flag_all_prepared:
                ...
                input_images = [
                    ...
                    for img_path in batch_examples["image_path"]
                ]
                inputs = self.processor(images=input_images, return_tensors="pt").to(
        ...
        for data_split in self.splits_to_process:
            ...
            ds_with_caption = ds.map(
                _caption_with_blip, batched=True, batch_size=128, writer_batch_size=128
            )
    ...
    def _call(self, inputs):
        ...
        def _caption_with_blip(batch_examples, rank, vision_model, image_processor):
            ...
            if flag_all_prepared:
                ...
                inputs = image_processor(images=input_images, return_tensors="pt").to(
...
class MergeDataColumns(HFDatasetTransform):
    ...
    def _call(self, inputs):
        ...
        for data_split in self.splits_to_process:
            ...
            cols_to_use = list(df_from.columns.difference(df.columns)) + [
                column_to_join_on
            ]
