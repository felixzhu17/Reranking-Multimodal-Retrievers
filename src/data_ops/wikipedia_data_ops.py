...
class LoadWikipediaPassageData(BaseTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        def add_title_to_passage(example):
            example['text'] = " ".join(["title:", example['title'], "content:", example['text']])
            return example
...
class LoadFullWikipediaPassageData(BaseTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        logger.info(f"After processing, there are {len(ds)} passages from {dataset_name} dataset")
...
class IndexPassagesWithElasticSearch(BaseTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        if not self.run_index:
            logger.warning("_run_index = False, Skipping indexing passages into ElasticSearch.")
            return self.data
        ...
        for data_split in ['train', 'test']:
            ...
            for item in tqdm(self.data.okvqa_data[data_split].data_items):
                ...
                if annotation is None:
                    ...
                    pos_item_contents = [
                        retrieved_passages[str(passage_id)]['text'] for passage_id in pos_item_ids
                    ]
                ...
                self.data.okvqa_data_with_dpr_output[data_split][str(question_id)] = knowledge_item
            ...
            ds_split = pd.DataFrame.from_dict(self.data.okvqa_data_with_dpr_output[data_split], orient='index')
