...
class PrepareDistillationScores(HFDatasetTransform):
    ...
    def _call(self, data, *args, **kwargs):
        ...
        query_tokenizer = FLMRQueryEncoderTokenizer.from_pretrained(
            checkpoint_path, subfolder="query_tokenizer"
        )
        ...
        for split in self.splits_to_process:
            ...
            def read_cached_data_from_es(batch):
                ...
                batch_ids_to_search = [
                    f"{self.source_name}_{question_id}"
                ...
                for doc in resp["docs"]:
                    if doc["found"]:
                        batch_neg_item_indices.append(
                            doc["_source"]["neg_item_indices"]
                        )
                        batch_neg_item_ids.append(
                            [
                                split_passages[i]["passage_id"]
                                for i in doc["_source"]["neg_item_indices"]
                            ]
                        )
                        batch_neg_item_contents.append(
                            [
                                split_passages[i]["passage_content"]
                                for i in doc["_source"]["neg_item_indices"]
                            ]
                        )
            ...
            def obtain_negatives(example):
                ...
                while len(neg_item_ids) < self.num_negatives:
                    ...
                    if neg_item_id not in pos_item_ids:
                        ...
                        neg_item_contents.append(
                            split_passages[neg_item_index]["passage_content"]
                        )
            ...
            def obtain_scores(examples, rank, model, image_processor):
                ...
                batch_neg_item_contents = [
                    [
                        split_passages[neg_item_index]["passage_content"]
                        for neg_item_index in neg_item_indices
                    ]
                    for neg_item_indices in batch_neg_item_indices
                ]
                batch_item_contents = [
                    [pos_item_contents[0]] + neg_item_contents
                    for pos_item_contents, neg_item_contents in zip(
                        batch_pos_item_contents, batch_neg_item_contents
                    )
                ]
                concat_item_contents = [
                    item for sublist in batch_item_contents for item in sublist
                ]
                ...
                with torch.no_grad():
                    ...
                    Q_pixel_values = image_processor(images, return_tensors="pt")[
                        "pixel_values"
                    ]
                    ...
                    inputs = dict(
                        query_input_ids=Q_encoding["input_ids"].to(
                            torch.device(f"cuda:{rank}")
                        ),
                        query_attention_mask=Q_encoding["attention_mask"].to(
                            torch.device(f"cuda:{rank}")
                        ),
                        query_pixel_values=Q_pixel_values.to(
                            torch.device(f"cuda:{rank}")
                        ),
                        context_input_ids=D_encoding["input_ids"].to(
                            torch.device(f"cuda:{rank}")
                        ),
                        context_attention_mask=D_encoding["attention_mask"].to(
                            torch.device(f"cuda:{rank}")
                        ),
                ...
                if self.use_elastic_search:
                    all_actions = []
                    for question_id, neg_item_indices, score_list in zip(
                        batch_question_ids, batch_neg_item_indices, examples["scores"]
                    ):
                        action = {
                            "_op_type": "index",
                            "_index": index_name,
            ...
            if self.limit_num_data is not None:
                ...
                if len(split_data) > self.limit_num_data:
                    print(
                        f"Limiting number of data to {self.limit_num_data} for split {split}..."
                    )
                    split_data = split_data.select(range(self.limit_num_data))

            split_data = split_data.map(
                read_cached_data_from_es,
                batched=True,
                load_from_cache_file=False,
                new_fingerprint="avoid_caching",
            )
            already_cached_data = split_data.filter(
                lambda example: example["found"],
                num_proc=16,
                load_from_cache_file=False,
            )
            print(f"Already cached data: {len(already_cached_data)}")
            remaining_data = split_data.filter(
                lambda example: not example["found"],
                num_proc=16,
                load_from_cache_file=False,
            )
            ...
            if len(remaining_data) > 0:
                print(
                    f"Obtaining negatives for {len(remaining_data)} remaining data..."
                )
                remaining_data = remaining_data.map(
                    obtain_negatives, num_proc=32, load_from_cache_file=False
                )
