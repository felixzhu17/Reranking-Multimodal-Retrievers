...
class LoadVisualGenomeData(BaseTransform):
    ...
    def _call(self, *args, **kwargs):
        ...
        logger.info(
            "[Data Statistics] VG data {}".format(len(self.data.vg_data.data_items))
        )
    ...
    def _call(self, inputs, **kwargs):
        ...
        for index, item in tqdm(enumerate(data_items)):
            ...
            for description_dict in entry_data["descriptions"]["regions"]:
                ...
                passage_id = self.data.passages["doc2id"].get(
                    description, str(len(self.data.passages["doc2id"]))
                )
        ...
        logger.info(
            "[Data Statistics] passages {}".format(len(self.data.passages["id2doc"]))
        )
        logger.info(
            "[Data Statistics] VG data train entries {}".format(
                len(self.data.vg_data.train["data_items"])
            )
        )
        logger.info(
            "[Data Statistics] VG data valid entries {}".format(
