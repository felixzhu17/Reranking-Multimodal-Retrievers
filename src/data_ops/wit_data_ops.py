...
class LoadWITData(HFDatasetTransform):
    ...
    def _call(self, *args, **kwargs):
        ...
        def add_image_id(batch):
            ...
            image_ids = [hashlib.md5(i.encode()).hexdigest() for i in image_urls]
        ...
        for split in self.data.wit_data.keys():
            ...
            for split_path in split_paths:
                ...
                if self.only_main_image:
                    ds = ds.filter(
                        lambda x: x["language"] == "en" and x["is_main_image"] == True
                    )
                else:
            ...
            split_ds = split_ds.map(
                add_image_id, batched=True, num_proc=16, desc="Add hashed id to images"
            )
            split_ds = split_ds.add_column(
                "original_data_id", [f"{split}_{i}" for i in range(len(split_ds))]
            )
...
class PrepareImagesForWITData(HFDatasetTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        for split in ["train", "valid", "test"]:
                ...
                for _ in range(retries + 1):
                    try:
                        ...
                        response = requests.get(
                            image_url, stream=True, timeout=timeout, headers=headers
                        )
                        ...
                        if os.path.exists(image_path):
                            try:
                                ...
                                if image.size[0] > 32 and image.size[1] > 32:
                                    ...
                                    logger.warning(
                                        f"Image size too small, removed {image_path} {image}"
                                    )
            ...
            valid_split_ds = split_ds.filter(
                lambda x: x["image_downloaded"] == True, num_proc=128
            )
            logger.info(
                f"After downloading, split {split} has {len(valid_split_ds)} entries."
            )
        ...
        output_data["images"] = pd.DataFrame.from_dict(
            output_data["images"], orient="index"
        ).set_index("id")
...
class PrepareImagesForWITDataFromPassages(BaseTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        def get_images(batch, num_threads, timeout=30, retries=3):
            image_urls = batch["image_url"]
            image_ids = batch["image_id"]
            image_paths = [os.path.join(image_data_path, f"{i}.jpg") for i in image_ids]
            batch["img_id"] = image_ids
            batch["img_path"] = image_paths
        ...
        logger.info(f"There are in total {len(output_data.images)} images.")
        logger.info(
            f"There are in total {len(output_data.passages.dataset)} documents."
        )
        return output_data
...
class LoadWITPassages(HFDatasetTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        full_corpus_ds = [
            split_ds
            for split, split_ds in self.data.items()
        ...
        def process_example(item):
            ...
            if item["hierarchical_section_title"] is not None:
                passage_content += (
                    f" hierarchical section title: {item['hierarchical_section_title']}"
                )
        ...
        output_data = DatasetDict({"passages": full_corpus_ds})
        return output_data
...
class TruncateWITPassages(BaseTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        output_data = EasyDict({"passages": self.data.passages})
        return output_data
...
class PrepareWITDataForRetrieval(HFDatasetTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        passages_ds = passages_ds.select_columns(
            ["original_data_id", "passage_id", "passage_content"]
        )
        ...
        for split in ["train", "valid", "test"]:
            ...
            if (
                iglue_test_file is not None
                and split == "valid"
                and (not self.use_dummy_data)
            ):
                # load json line file
                ...
                iglue_match_tuples = [
                    (i, j, k)
                    for i, j, k in zip(
                        iglue_all_page_urls_list,
                        iglue_all_image_urls_list,
                        iglue_all_captions_list,
                    )
                ]
                ...
                split_ds = split_ds.filter(
                    lambda x: iglue_match_dict.get(
                        (
                            x["page_url"],
                            x["image_url"],
                            x["caption_reference_description"],
                        ),
                        None,
                    )
                    is not None
                ...
                logger.info(
                    f"After filtering, there are {len(split_ds)} examples in split {split}."
                )
            ...
            split_df = split_df.join(
                passages_df.set_index("original_data_id"),
                on="original_data_id",
                how="left",
            )
            ...
            split_df = split_df.groupby("image_id").progress_aggregate(
                agg_func
            )  # .progress_aggregate(pos_item_ids=pd.NamedAgg(column="passage_id", aggfunc=agg_func))
        ...
        logger.info("[Data Statistics] passages {}".format(len(self.data["passages"])))
        logger.info(
            "[Data Statistics] WIT data train entries {}".format(
                len(output_data["train"])
            )
        )
        logger.info(
            "[Data Statistics] WIT data valid entries {}".format(
                len(output_data["valid"])
            )
        )
        logger.info(
            "[Data Statistics] WIT data test entries {}".format(
...
class RemoveWITPassagesWithoutImages(BaseTransform):
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        self.data.passages.dataset = full_ds.filter(
            lambda x: self.data.passages.available_documents.get(x["passage_id"], None)
            is not None,
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for data_split in ["train", "test"]:
            ...
            for item in tqdm(self.data.okvqa_data[data_split].data_items):
                ...
                knowledge_collection = [i[2] for i in res]
    ...
    def _call(self, inputs, *args, **kwargs):
        ...
        for input_data in inputs:
            ...
            if "image_dataset_with_embeddings" in input_data.keys():
                print(
                    "input image_dataset_with_embeddings:",
                    len(input_data.image_dataset_with_embeddings),
                )
                full_image_dataset_with_embeddings.append(
                    input_data.image_dataset_with_embeddings
                )
                print(input_data.image_dataset_with_embeddings[:1])

        # concatenate image dataset
        full_image_dataset_with_embeddings = concatenate_datasets(
            full_image_dataset_with_embeddings
        )
...
class SplitWITPassagesForLargeScaleTraining(HFDatasetTransform):
    ...
    def _call(self, inputs, **kwargs):
        ...
        ds = full_corpus_ds.filter(
            lambda x: valid_passage_ids.get(x["passage_id"], None) is not None
        )
