...
def parse_sys_args(args_list=None):
    ...
    arg_parser.add_argument(
        ...
        type=str,
        default="",
        help="The Experiment name from which the new experiment inherits/overwrites config",
    )
    arg_parser.add_argument(
        "--test_suffix",
        type=str,
        default="",
        help="Tests will be saved under /path/to/EXPERIMENT_FOLDER/$experiment_name$/test/$test_suffix$.",
    )

    arg_parser.add_argument(
        "--mode", type=str, default="prepare_data", help="prepare_data/train/test"
    )
    arg_parser.add_argument(
        "--reset",
        action="store_true",
