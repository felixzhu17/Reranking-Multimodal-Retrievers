...
options="--experiment_name \"OKVQA_VisualColBERT_with_pretrained_ViT(WIT)_ColBERT_mapping_trainable_ViT_frozen_10ROI_with_text_based_vision\" \
    --config \"configs/okvqa/ColBERT_vision_with_ROI.jsonnet\" \
    --accelerator auto --devices auto --strategy ddp \
    --mode test \
    --test_suffix \"generate_index_train\" \
    --accelerator auto --devices auto --strategy ddp \
    --opts model_config.full_corpus_in_training=1 \
           model_config.full_corpus_in_testing=1 \
           model_config.num_ROIs=9 \
           test.batch_size=16 \
           test.checkpoint_name=\"model_step_11036.ckpt\""
