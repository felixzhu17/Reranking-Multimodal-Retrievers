...
class FLMRExecutor(BaseExecutor, MetricsProcessor):
    ...
    def _init_model(self, model_config): 
        ...
        QueryEncoderConfigClass = globals()[model_config.QueryEncoderConfigClass]
        QueryEncoderModelClass = globals()[model_config.QueryEncoderModelClass]
        QueryEncoderModelVersion = model_config.QueryEncoderModelVersion

        ContextEncoderConfigClass = globals()[model_config.ContextEncoderConfigClass]
        ContextEncoderModelClass = globals()[model_config.ContextEncoderModelClass]
        ContextEncoderModelVersion = model_config.ContextEncoderModelVersion

        query_encoder_config = QueryEncoderConfigClass.from_pretrained(QueryEncoderModelVersion, trust_remote_code=True)
        context_encoder_config = ContextEncoderConfigClass.from_pretrained(ContextEncoderModelVersion, trust_remote_code=True)

        context_encoder_config.load_cpu_extension = True

        self.use_dtype = torch.bfloat16

        query_encoder = QueryEncoderModelClass.from_pretrained(QueryEncoderModelVersion, 
            config=query_encoder_config, 
        ...
        if model_config.ContextEncoderModelClass == "FLMRModelForRetrieval":
            flmr_query_tokenizer = FLMRQueryEncoderTokenizer.from_pretrained(
                ContextEncoderModelVersion, subfolder="query_tokenizer"
            )
            ...
            context_encoder = ContextEncoderModelClass.from_pretrained(ContextEncoderModelVersion, config=context_encoder_config, trust_remote_code=True)
        ...
        # Enable LORA
        # self.model.query_encoder.transformer.visual.requires_grad_(False)
        # if hasattr(self.model.query_encoder.transformer.visual,'attn_pool'):
        #     self.model.query_encoder.transformer.visual.attn_pool.requires_grad_(True)
    ...
    def training_step(self, sample_batched, batch_idx):
        ...
        image_sizes = sample_batched.get('image_sizes', None)
        if image_sizes is not None:
            train_batch["query_image_sizes"] = image_sizes.to(self.device)
    ...
    def _compute_loss(self, sample_batched, batch_idx, dataloader_idx=0):
        ...
        image_sizes = sample_batched.get('image_sizes', None)
        if image_sizes is not None:
            test_batch["query_image_sizes"] = image_sizes.to(self.device)
        ...
        if use_index is None:
            ...
            if self.global_rank == 0 and not exhaustive_search_in_testing:
                ...
                index_custom_collection(
                    ...
                    model=self.model.context_encoder,
    ...
    def evaluate_outputs(self, step_outputs, current_data_loader, dataset_name, dataloader_idx=0, mode='test'):
        ...
        for step_output in step_outputs:
            ...
            for mask in step_output['question_mask']:
                question_masks.append(mask)
            for mask in step_output['image_mask']:
                image_masks.append(mask)
            for mask in step_output['instruction_mask']:
                instruction_masks.append(mask)
            
        
        Q_for_candidate_generation = []

        # pad each query embeddings with zero tensors to make them have the same length 
        for i in range(len(query_embeddings)):
            # print(f"before query_embeddings[{i}]", query_embeddings[i].shape)
            query_embeddings[i] = torch.cat([query_embeddings[i], torch.zeros(query_embeddings[i].shape[0], max_query_emb_len - query_embeddings[i].shape[1], query_embeddings[i].shape[2])], dim=1)
            # print(f"query_embeddings[{i}]", query_embeddings[i].shape)

        query_embeddings = torch.cat(query_embeddings, dim=0)

        for i in range(len(query_embeddings)):
            # print(question_masks[i].cpu().numpy().tolist())
            # candidate_mask = question_masks[i] == 1
            # candidate_mask = (question_masks[i] + image_masks[i]) == 1
            # print("before", query_embeddings[i].shape, candidate_mask.shape)
            # candidate_embeddings = query_embeddings[i][candidate_mask]
            # print("after", candidate_embeddings.shape)
            start_pos = torch.where(question_masks[i] == 1)[0][0]
            end_pos = torch.where(question_masks[i] == 1)[0][-1]
            # # print("start_pos", start_pos, "end_pos", end_pos)
            Q_for_candidate_generation.append(query_embeddings[i][start_pos:end_pos+1])
            # Q_for_candidate_generation.append(candidate_embeddings)
        ...
        if "exhaustive_search_in_testing" not in self.model_config.modules:
            ...
            ranking = search_custom_collection(
                ...
                Q_for_candidate_generation=Q_for_candidate_generation,
            )
